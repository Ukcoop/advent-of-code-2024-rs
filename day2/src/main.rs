...
fn is_safe(report: Vec<u32>) -> bool {
    ...
    for i in 0..(report.len() - 1) {
        ...
        if i > 0 && flip != (report[i] > report[i + 1]) { return false; }

        let sorted: Vec<u32> = sort(vec![report[i], report[i + 1]]);
        if sorted[1] - sorted[0] > 3 { return false; }
    }
...
fn main() {
    ...
    let safe_reactors = reactors_safe(reports.clone(), false);
